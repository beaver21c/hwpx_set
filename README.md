...
서식을 프로파일로 옮겨 적어 새로 지으면(④·`extract`), 한글의 **자동 글머리표·
번호매기기**처럼 문단 속성 바깥에 있는 것을 잃는다. ①은 반대로 간다. 양식 파일을
**템플릿으로 그대로 두고** 본문 문단만 갈아 끼운다. `header.xml`을 한 바이트도
건드리지 않으므로 글꼴·자동 글머리표·번호매기기·쪽 설정이 원본 그대로다.
서식이 재현되는 것이 아니라 **보존**된다.
...
```
양식.hwpx  ─→  해부  ─→  꾸러미(양식 원본 + 빌더 + 스킬)  ─→  AI가 쓰고 만든다
