> **이 문서는 착수 시점의 설계 원안이다.** 지금 구현과 다른 점을 아래에 적어 둔다.
> 쓰는 법이 아니라 왜 이렇게 만들었는지를 남기는 문서다.
>
> **원안대로 간 것** — 레벨 개수 가변, 도식 기본 렌더는 표, 화살표는 `→`/`▼` 문자 +
> 이미지 폴백.
>
> **원안과 달라진 것**
>
> - `profiles/`를 `hwpx_studio/profiles/`로 옮겼다(설치본에 함께 넣기 위함).
>   `templates/`의 SKILL 템플릿·prompt 조각은 별도 파일 대신 `export_skill.py`에 넣었다
> - §7.3 org 레이아웃은 원안(리프 수 × 2 + 1, 상자 1칸)과 달리 **균일 폭 열 격자**다.
>   상자를 짝수 개 열로 병합해 그 경계를 중심선으로 삼으므로 연결선이 상자 **가운데**에
>   맞는다(원안·프로토타입은 상자 왼쪽 변에 붙었다). 근거: `docs/diagram-guide.md` §4
> - 셀 한 변 테두리로 그린 연결선이 한글에서 정상 표시되는 것을 확인했다(한글 모바일
>   뷰어). §7.4의 대안(배경색 셀 대체)은 필요 없다 — `prototypes/README.md`
>
> **원안 밖에서 더한 것**
>
> - **양식 보존**: 원안은 서식을 프로파일로 옮겨 적어 문서를 새로 짓는 길 하나뿐이었다.
>   그 길로는 한글의 자동 글머리표·번호매기기(`paraPr`의 `hh:heading`)를 옮기지 못한다.
>   그래서 양식 파일을 **템플릿으로 그대로 두고 본문 문단만 갈아 끼우는** 길을 더했다
>   (`formkit.py` → `assets/build_form.py`). `header.xml`을 한 바이트도 건드리지 않으므로
>   서식이 재현이 아니라 **보존**된다. 근거·한계는 `docs/form-guide.md`
> - **되돌리기**: 서식 없는 hwpx를 마커 텍스트로 되돌린다(`assets/read_hwpx.py`).
>   줄머리 기호·번호·글자 크기를 근거로 계층을 **추정**하고 근거를 보고서로 남긴다.
>   그림은 읽지 못한다
> - **브라우저 앱**: 네 갈래(양식 꾸러미·도식·되돌리기·바로 쓰기)로 나뉜다. 해부·되돌리기는
>   브라우저에도 이식했고 `tools/compare_js_python.py`가 두 엔진의 `form.json`과 마커
>   텍스트를 대조한다
> - **실제 문서 확인 2건**: 한글로 직접 작성한 hwpx에서 7레벨 전부 복원(그 과정에 드러난
>   결함 3건을 고치고 `tests/test_extractor_real.py`로 고정), 기관 서식 1건에서 용지·여백·
>   각주 모양을 기관 실측표와 대조. **검사에 쓴 실제 문서는 저장소에 넣지 않는다**
...
│   ├── gov-3level.json            # □/○/- 3레벨
│   ├── narrative.json             # 서술식
│   └── report-crown.json       # 연구보고서(제N장·제N절, 장별 표·그림 번호)
